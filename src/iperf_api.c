...
void iperf_set_test_bitrate_limit_maximum(struct iperf_test *ipt, uint64_t total_rate) {
    ipt->settings->bitrate_limit = total_rate;
}

void iperf_set_test_bitrate_limit_interval(struct iperf_test *ipt, uint64_t bitrate_limit_interval) {
    ipt->settings->bitrate_limit_interval = bitrate_limit_interval;
}

void iperf_set_test_bitrate_limit_stats_per_interval(struct iperf_test *ipt, uint64_t bitrate_limit_stats_per_interval) {
...
void iperf_set_test_bind_address(struct iperf_test *ipt, const char *bnd_address) {
 ...
 */
int iperf_parse_hostname(struct iperf_test *test, char *spec, char **p, char **p1) {
    ...
    if ((*p = strtok(spec, "%")) != NULL && (*p1 = strtok(NULL, "%")) != NULL) {
         ...
         */
        if (inet_pton(AF_INET6, *p, &ipv6_addr) == 1 && IN6_IS_ADDR_LINKLOCAL(&ipv6_addr)) {
            if (test->debug) {
...
int iperf_parse_arguments(struct iperf_test *test, int argc, char **argv) {
    ...
    while ((flag = getopt_long(argc, argv, "p:f:i:D1VJvsc:ub:t:n:k:l:P:Rw:B:M:N46S:L:ZO:F:A:T:C:dI:hX:", longopts, NULL)) != -1) {
        ...
        case 'f':
            ...
            if (test->settings->unit_format == 'k' || test->settings->unit_format == 'K' || test->settings->unit_format == 'm' || test->settings->unit_format == 'M' || test->settings->unit_format == 'g' || test->settings->unit_format == 'G' || test->settings->unit_format == 't' || test->settings->unit_format == 'T') {
                break;
            } else {
                i_errno = IEBADFORMAT;
                return -1;
    ...
    if (test->role == 's' && (client_username || client_rsa_public_key)) {
        i_errno = IECLIENTONLY;
        return -1;
    } else if (test->role == 'c' && (client_username || client_rsa_public_key) && !(client_username && client_rsa_public_key)) {
    ...
    if (test->role == 'c' && (server_rsa_private_key || test->server_authorized_users)) {
        ...
        return -1;
    } else if (test->role == 's' && (server_rsa_private_key || test->server_authorized_users) && !(server_rsa_private_key && test->server_authorized_users)) {
    ...
    if (test->settings->bitrate_limit_interval != 0) {
        test->settings->bitrate_limit_stats_per_interval =
            (test->settings->bitrate_limit_interval <= test->stats_interval ?
                 1 :
                 round(test->settings->bitrate_limit_interval / test->stats_interval));
    }
...
void iperf_check_total_rate(struct iperf_test *test, iperf_size_t last_interval_bytes_transferred) {
    double seconds;
    uint64_t bits_per_second;
...
int iperf_send(struct iperf_test *test, fd_set *write_setP) {
    ...
    for (; multisend > 0; --multisend) {
        ...
        SLIST_FOREACH(sp, &test->streams, streams) {
            if ((sp->green_light && sp->sender && (write_setP == NULL || FD_ISSET(sp->socket, write_setP)))) {
...
static void send_timer_proc(TimerClientData client_data, struct iperf_time *nowP) {
...
static void iperf_print_intermediate(struct iperf_test *test) {
    ...
    for (current_mode = lower_mode; current_mode <= upper_mode; ++current_mode) {
        ...
        if (test->num_streams > 1 || test->json_output) {
            ...
            sum_name = "sum";
            if (test->mode == BIDIRECTIONAL) {
                if ((test->role == 'c' && !stream_must_be_sender) || (test->role != 'c' && stream_must_be_sender)) {
...
/**************************************************************************/
struct iperf_stream * iperf_new_stream(struct iperf_test *test, int s, int sender) {
    struct iperf_stream *sp;
    int ret = 0;
...
int iperf_init_stream(struct iperf_stream *sp, struct iperf_test *test) {
    ...
    if (iperf_get_test_protocol_id(test) == Pudp && getsockdomain(sp->socket) == AF_INET && iperf_get_dont_fragment(test)) {
        /*
...
static int diskfile_send(struct iperf_stream *sp) {
    ...
    if (sp->diskfile_left < sp->test->settings->blksize && !sp->test->done) {
        r = read(sp->diskfile_fd, sp->buffer, sp->test->settings->blksize - sp->diskfile_left);
        buffer_left += r;
        rtot += r;
        if (sp->test->debug) {
