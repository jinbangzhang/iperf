    ...
    */
    ...
    if (test->settings->bitrate_limit_interval != 0)
    {
        test->settings->bitrate_limit_stats_per_interval =
            (test->settings->bitrate_limit_interval <= test->stats_interval ? 1 : round(test->settings->bitrate_limit_interval / test->stats_interval));
    }
...
int iperf_send(struct iperf_test *test, fd_set *write_setP)
{
    ...
    if (!no_throttle_check)
    { /* Throttle check if was not checked for each send */
        iperf_time_now(&now);
        SLIST_FOREACH(sp, &test->streams, streams)
        if (sp->sender)
            iperf_check_throttle(sp, &now);
...
int iperf_clearaffinity(struct iperf_test *test)
{
    ...
    if (GetProcessAffinityMask(process, &processAffinityMask, &lpSystemAffinityMask) == 0 || SetProcessAffinityMask(process, lpSystemAffinityMask) == 0)
    {
        i_errno = IEAFFINITY;
        return -1;
    }
